...
class DatasetClass(object):
    def __init__(self,cfg):
        ...
        if 'kappa' in cfg.prior.keys():
            self.kappa = self.get_kappa(image,
                cfg.prior.kappa,
                cfg.dataset.kappa
                )

        self.initial = self.get_initial(image,
                cfg.dataset.initial.use,
                cfg.dataset.initial.path
                )
    ...
    def get_kappa(self, image, kappa_use, kappa_path):
        if kappa_use:
            return image.clone().fill(pet.ImageData(kappa_path))
        else:
            return image.clone().fill(1)

    def get_initial(self, image, initial_use, initial_path):
        if initial_use:
            return image.clone().fill(pet.ImageData(initial_path))
        else:
            return image.clone().fill(1)

def Dataset2D(prompts,additive,multiplicative,image_xy):
    # GET THE DATA
