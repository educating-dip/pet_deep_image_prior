...
def CRC(ROIs_b, ROIs_a, recon, gt):
    ...
    abar = 1/a.K * np.sum((reconstruction * a.mask).ravel())
    bbar = 1/b.K * np.sum((reconstruction * b.mask).ravel())
    atrue = 1/a.K * np.sum((ground_truth * a.mask).ravel())
    btrue = 1/b.K * np.sum((ground_truth * b.mask).ravel())
    return 1/R * (abar/bbar - 1) / (atrue/btrue - 1)

def STD(ROIs_b, ROIs_a, recon, gt):
    # STANDARD DEVIATION
    # abar = ROI average uptake
    # Ka = number of ROIs
    # bbar = background average uptake
...
@hydra.main(config_path='../cfgs', config_name='baselines')
def baselines(cfg : DictConfig) -> None:
    ...
    if cfg.dataset.name == "2D":
        ...
        ROIs_masks = []
        ROIs_b = np.load(cfg.dataset.quality_path + "/" + "ROI_BackgroundVoxel" + ".npy")
        for i in range(len(ROIs)):
            ROIs_masks.append(np.load(cfg.dataset.quality_path + "/" + ROIs[i] + ".npy"))
        print(np.shape(ROIs_masks))
    elif cfg.dataset.name == "3D":
        ROIs = ["ROI_AbdominalWallLesion","ROI_HeartLesion","ROI_Liver","ROI_LiverLesion","ROI_LungLesion","ROI_SpineLesion"]
        ROIs_masks = []
        ROIs_b = np.load(cfg.dataset.quality_path + "/" + "ROI_BackgroundVoxel" + ".npy")
        for i in range(len(ROIs)):
            ROIs_masks.append(np.load(cfg.dataset.quality_path + "/" + ROIs[i] + ".npy"))
    ...
    for i in range(0, cfg.dataset.num_subsets*cfg.dataset.num_epochs + 1):
        ...
        writer.add_scalar("CRC",CRC(ROIs_b, ROIs_masks,current_image.as_array()),i)
        writer.add_scalar("STDEV", STD(ROIs_b, ROIs_masks,current_image.as_array()),i)
